...
class Html extends \lithium\template\Helper {
	...
	protected $_strings = array(
		...
		'block-start'      => '<div{:options}>',
                'button-link'      => '<button onclick="javascript:window.location=\'{:url}\'"{:options}>{:title}</button>',
	...
	public function link($title, $url = null, array $options = array()) {
        ...
        public function buttonLink($title, $url = null, array $options = array()) {
            $defaults = array('escape' => true, 'type' => null);
            list($scope, $options) = $this->_options($defaults, $options);

            $url = is_null($url) ? $title : $url;
            return $this->_render(__METHOD__, 'button-link', compact('title', 'url', 'options'), $scope);
        }
